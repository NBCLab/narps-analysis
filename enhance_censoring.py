...
def censor(metric_arr, metric_thresh, n_contig=2, n_before=1, n_after=2):
    """
    Censor a numpy array based on a threshold, then enhance the censoring.
    """
    censor_vec = metric_arr >= metric_thresh
    out_vec = enhance(censor_vec, n_contig=n_contig, n_before=n_before, n_after=n_after)
    return out_vec


def enhance(censor_vec, n_contig=2, n_before=1, n_after=2):
    """
    Enhance a censoring vector of 1s (bad) and 0s (good)
    by censoring volumes before, after, and sometimes between
    censored volumes.
    """
...
if __name__ == '__main__':
    import sys
    args = vars(_get_parser().parse_args(sys.argv[1:]))
    main(**args)
